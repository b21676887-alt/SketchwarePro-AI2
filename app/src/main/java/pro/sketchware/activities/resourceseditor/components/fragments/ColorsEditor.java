...
public class ColorsEditor extends Fragment {
	...
	public void showColorEditDialog(ColorModel colorModel, int position) {
		...
		dialogBinding.colorPreviewCard.setOnClickListener(v -> {
			String inputColor = Objects.requireNonNull(dialogBinding.colorValueInput.getText()).toString().trim();
			String colorToPass = inputColor;
			
			// التحقق من صحة اللون إذا كان كود Hex
			if (inputColor.startsWith("#")) {
				try {
					Color.parseColor(inputColor);
				} catch (Exception e) {
					colorToPass = "#FFFFFF"; // تمرير لون افتراضي في حال كان الكود غير صالح مثل #a
				}
			} else if (inputColor.isEmpty()) {
				colorToPass = "#FFFFFF";
			}
			
			ColorPickerDialog colorPicker = new ColorPickerDialog(activity, colorToPass, false, false, activity.sc_id);
			colorPicker.a(new ColorPickerDialog.b() {
				...
				@Override
				public void a(String resName, int colorInt) {
					dialogBinding.colorValueInput.setText("@color/" + resName);
					dialogBinding.colorPreviewCard.setCardBackgroundColor(colorInt);
				}
			});
			colorPicker.materialColorAttr((attr, attrColor) -> {
				dialogBinding.colorValueInput.setText("?" + attr);
				dialogBinding.colorPreviewCard.setCardBackgroundColor(attrColor);
			});
